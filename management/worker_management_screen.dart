...
class _WorkerManagementScreenState extends State<WorkerManagementScreen> {
  ...
  /// Calculate total worker costs
  void _calculateTotalWorkerCost() {
    totalWorkerCost = 0.0;
    for (var worker in workers) {
      final salary = (worker['salary'] as num?)?.toDouble() ?? 0.0;
      totalWorkerCost += salary;
    }
  }
  ...
  /// Update worker salary only
  Future<void> _updateWorkerSalary({
    required int id,
          ...
          TextButton(
            onPressed: () {
              ...
              if (salary < 0) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Salary cannot be negative'),
                    backgroundColor: Colors.red,
                  ),
                );
                return;
  ...
  void _showEditWorkerDialog(Map<String, dynamic> worker) {
    ...
    final salaryController = TextEditingController(
      text: ((worker['salary'] as num?)?.toDouble() ?? 0.0).toStringAsFixed(2),
    );
  ...
  /// Show edit salary dialog
  void _showEditSalaryDialog(Map<String, dynamic> worker) {
    final salaryController = TextEditingController(
      text: ((worker['salary'] as num?)?.toDouble() ?? 0.0).toStringAsFixed(2),
    );
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'Salary: ৳${salary.toStringAsFixed(2)}/mo',
                      style: const TextStyle(
                        color: primary,
                        fontWeight: FontWeight.bold,
                        fontSize: 12,
              ...
              Column(
                children: [
                  ...
                  GestureDetector(
                    onTap: () => _showEditSalaryDialog(worker),
                    child: Container(
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          gradient: LinearGradient(
                            ...
                            colors: [
                              costAlert.withOpacity(0.2),
                              costAlert.withOpacity(0.05)
                            ],
                          ),
                          borderRadius: BorderRadius.circular(12),
                          border: Border.all(color: costAlert.withOpacity(0.3)),
                        ),
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 4),
                                Text(
                                  '৳${totalWorkerCost.toStringAsFixed(2)}',
                                  style: const TextStyle(
                                    color: Colors.white,
                                    fontSize: 24,
                                    fontWeight: FontWeight.bold,
                            ...
                            Icon(
                              ...
                              color: costAlert.withOpacity(0.5),
                              size: 40,
                            ),
                          ],
                        ),
